...
<head>
	...
	<title>3x10 Basket Ball Score Tracker</title>
	...
	function fetchUsers()
	{
		...
		$.ajax({
				async: false,
				type: "GET", // We want to get data from server (no modifications, etc)
				url: 'getUsers.php',
				data: '&ts='+$.now(),
	...
	function populateForm()
	{
		...
		$("#scorerSelect,#shooterSelect").each(function(){
			$(this).change(function(){
				...
				{
					$(this).replaceWith('<input type="text" size="10" id="'+$(this).attr('id')+'" name="'+$(this).attr('id')+'"/>');
				}
				else if( $(this).val() != '' )
				{
		...
		$("#round1Select,#round2Select,#round3Select").each(function(){
			$(this).change(function(){
				$("#totalScore").val( parseInt($("#round1Select").val())+parseInt($("#round2Select").val())+parseInt($("#round3Select").val()) );
				if( $(this).val() != '' )
				{
					$(this).removeClass("errorInput");
				}
	...
	function parseBasicStats(stats)
	{
		...
		return "<b>"+parts[0]+"</b> has <b>"+parts[1]+"</b> / <b>"+parts[2]+"</b> baskets for <b>"+Math.floor((parts[1] / parts[2]) * 100)+"%</b>"+'\n';
	}
	function showStats()
	{
		$("#returnedStats").text("");
	...
	$(document).ready(function()
	{
		...
		$("#scoreForm").submit(function(event) {
			...
			else
			{
				...
				request = $.ajax({
						type: "POST",
						url: "submitScores.php", 			// the script where you handle the form input.
						data: {
	...
	<div id='wrapper'>
		...
		<div id='main'>
			...
			<div id='content' align='center'>
				<div id='stats' hidden>
					Here, You will be able to view some stats<hr/>
					<button id="refreshStats" type="button">Refresh Stats</button><hr/>
				...
				<div id='enterscore' hidden>
					...
					Round 3: <select id='round3Select' name='round3Select'></select>/10<hr width="150px"/>
					Total:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id='totalScore' name='totalScore' value='0' maxlength="2" size="1" disabled></select>/30<br/>
